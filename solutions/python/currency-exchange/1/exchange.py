...
def exchangeable_value(budget: float, exchange_rate: float, spread: int, denomination: int) -> int:
    ...
    # 1. Calculate actual exchange rate including the spread fee
    actual_rate = exchange_rate * (1 + spread / 100)
    
    # 2. Find total possible currency you can buy
    total_currency = budget / actual_rate
    
    # 3. Figure out how many full bills (denominations) fit into that total
    number_of_bills = total_currency // denomination
    
    # 4. Multiply back to get the final integer value
